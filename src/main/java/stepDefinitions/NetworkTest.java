...
public class NetworkTest extends BaseDriver {
    ...
    @Given("The network url is checked.")
    ...
    @Given("Search components “jacket” are printed and searched.")
    public void search_components_jacket_are_printed_and_searched() {
        networkPage.inputJacket();
    }
    @Given("Scrolling as much as the show more button on the product listing page.")
    public void scrolling_as_much_as_the_show_more_button_on_the_product_listing_page() {
        networkPage.showMore();
    }
    @Given("It is checked that the 2nd page is passed \\(url changes).")
    ...
    @Given("You hover over the first discounted product on the product list page, a random size selection is made.")
    ...
    @Given("Hover the Go to Cart button that opens in the lower right corner.")
    public void hover_the_go_to_cart_button_that_opens_in_the_lower_right_corner() {
        networkPage.goToCard();
    }
    @Given("It is checked that the size and price information of the product is correct in the basket.")
    ...
    @Given("It is checked that the old price of the product added to the cart is greater than the discounted price.")
    ...
    @Given("Continue button is clicked.")
    public void continue_button_is_clicked() {
        networkPage.goOn();
    }
    @Given("Filling in the user information from csv format \\(E-mail - by reading the password from the csv file is necessary.)")
